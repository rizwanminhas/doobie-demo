...
object DoobieDemo extends IOApp {
  ...
  def saveActor(id: Int, name: String): IO[Int] = { // IO[Int] = number of affected rows
    val query = sql"insert into actors (id, name) values ($id, $name)"
    query.update.run.transact(xa)
  }

  def saveActor_v2(id: Int, name: String): IO[Int] = { // IO[Int] = number of affected rows
    val query = "insert into actors (id, name) values (?, ?)"
    Update[Actor](query).run(Actor(id, name)).transact(xa)
  }

  // auto generated IDs
  def saveActorAutoGenerated(name: String): IO[Int] = { // IO[Int] = auto generated key
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    //saveActor(99, "Rizwan").debug.as(ExitCode.Success)
    //saveActorAutoGenerated("Rizwan Minhas").debug.as(ExitCode.Success)
    //saveActor_v2(8, "Minhas").debug.as(ExitCode.Success)
    saveMultipleActors(List("abc", "def", "ghi")).debug.as(ExitCode.Success)
  }
}
